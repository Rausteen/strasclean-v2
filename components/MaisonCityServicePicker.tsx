          ...
          return (
            <button
              ...
              className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${
                active
                  ? "border-amber-400/60 bg-amber-400 text-ink-950 shadow"
                  : "border-white/10 bg-white/[0.04] text-white/70 hover:border-amber-400/30 hover:text-white"
              }`}
            >
              {s.shortName}
            </button>
        ...
        {CITIES.map((c) => (
          <li key={c.slug}>
            <Link
              ...
              prefetch={false}
              className="group flex items-center justify-between gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2.5 text-sm text-white/85 transition hover:border-amber-400/40 hover:bg-amber-500/10 hover:text-white"
            >
