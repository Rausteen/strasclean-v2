    ...
    <section className="relative py-14 sm:py-24 lg:py-28">
      <div className="container-x">
        <Reveal className="mx-auto max-w-3xl text-center">
          ...
          <p className="mt-4 text-slate-700">{service.introTemplate(city)}</p>
        </Reveal>
        ...
        <div className="mt-10 grid gap-6 sm:mt-12 lg:grid-cols-2">
            ...
            <div className="h-full rounded-3xl border border-slate-200 bg-slate-50 p-7 sm:p-8">
              ...
              <ul className="mt-5 space-y-3">
                {service.whatsIncluded.map((item) => (
                  ...
                  >
                    <span className="mt-0.5 grid h-5 w-5 shrink-0 place-items-center rounded-full bg-brand-500 text-slate-900">
                      <CheckIcon size={12} />
                    </span>
                    {item}
                  </li>
              ...
              <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-2 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
                <span>
                  Durée : <strong className="text-slate-900">{service.duration}</strong>
                </span>
                <span className="text-slate-300">·</span>
                <span>
                  À partir de{" "}
                  <strong className="text-slate-900">{service.priceFrom} €</strong>
                </span>
              </div>
            </div>
          </Reveal>
          ...
          {/* Pourquoi */}
          <Reveal delay={120}>
            <div className="relative h-full overflow-hidden rounded-3xl border border-slate-200 bg-gradient-to-br from-brand-500/10 to-slate-50 p-7 sm:p-8">
              ...
              <ul className="mt-5 space-y-4">
                {service.whyChoose.map((w) => (
                  <li key={w.title} className="flex gap-4">
                    <span className="mt-0.5 grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-brand-500/15 text-brand-600">
                      <CheckIcon size={16} />
                    </span>
                    <div>
                      <p className="text-sm font-semibold text-slate-900">{w.title}</p>
                      <p className="mt-0.5 text-sm text-slate-600">{w.desc}</p>
                    </div>
                  </li>
                ))}
              </ul>
